...
public class MixinHumanoidArmorLayer<T extends LivingEntity, M extends HumanoidModel<T>, A extends HumanoidModel<T>>
{
    ...
    {
        this.renderArmorModified(_matrixStackIn, _bufferIn, entityLivingBaseIn, _packedLightIn, armorItem, glintIn, modelIn, legs, red, green, blue, armorResource);
    }

    private void renderArmorModified(PoseStack matrixStack, MultiBufferSource buffer, T entity, int packedLight, ArmorItem armorItem, boolean glint, A model, boolean legs, float red, float green, float blue, String armorResource)
    {
        VertexConsumer ivertexbuilder = ItemRenderer.getArmorFoilBuffer(buffer, RenderType.armorCutoutNoCull(this.getArmorLocation(armorItem, legs, armorResource)), false, glint);
