...
public class CapeRendererHandler
{
    ...
    public void onPreRender(RenderPlayerEvent.Specials.Pre event)
    {
        ...
        if (ConfigManager.enableCustomCape && player.getCommandSenderName().equals(GameProfileUtil.getUsername()) && !player.isInvisible() && ExtendedConfig.SHOW_CAPE && !CapeUtil.CAPE_TEXTURE.isEmpty())
        {
            ...
            f6 = MathHelper.clamp_float(f6, 0.0F, 150.0F);
            f7 = MathHelper.clamp_float(f7, -20.0F, 20.0F);
        ...
        else
        {
            ...
            {
                IndicatiaMod.MC.getTextureManager().bindTexture(player.getLocationCape());
                GL11.glColor3f(1.0F, 1.0F, 1.0F);
                GL11.glPushMatrix();
                GL11.glTranslatef(0.0F, 0.0F, 0.125F);
                double d3 = player.field_71091_bM + (player.field_71094_bP - player.field_71091_bM) * event.partialRenderTick - (player.prevPosX + (player.posX - player.prevPosX) * event.partialRenderTick);
