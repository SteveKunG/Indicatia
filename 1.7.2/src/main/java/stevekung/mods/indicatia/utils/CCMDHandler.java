...
public class CCMDHandler extends ClientCommandHandler
{
    @Override
    public int executeCommand(ICommandSender sender, String message)
    {
        ...
        {
            sender.addChatMessage(this.format(EnumChatFormatting.RED, "commands.generic.usage", this.format(EnumChatFormatting.RED, wue.getMessage(), wue.getErrorOjbects())));
        }
        catch (CommandException ce)
        {
            sender.addChatMessage(this.format(EnumChatFormatting.RED, ce.getMessage(), ce.getErrorOjbects()));
        }
        catch (Throwable t)
        {
    ...
    public void autoComplete(String leftOfCursor, String full)
    {
        ...
        if (leftOfCursor.charAt(0) == '/')
        {
            ...
            if (mc.currentScreen instanceof GuiChat)
            {
                ...
                List<String> commands = this.getPossibleCommands(mc.thePlayer, leftOfCursor);
