...
@OnlyIn(Dist.CLIENT)
public class PlayerTabOverlayGuiIN extends PlayerTabOverlayGui
{
        ...
        for (int k4 = 0; k4 < playerListSize; ++k4)
        {
            ...
            if (k4 < list.size())
            {
                ...
                else
                {
                    ...
                    if (s4.contains("YOU"))
                    {
                        PlayerTabOverlayGuiIN.murderAssassinsNick = TextFormatting.getTextWithoutFormattingCodes(s4).replace("YOU ", "");
                    }
    ...
    protected void drawPing(int x1, int x2, int y, NetworkPlayerInfo info)
    {
        ...
        if (InfoUtils.INSTANCE.isHypixel() && (info.getGameProfile().getName().equals(ExtendedConfig.instance.hypixelNickName) || info.getGameProfile().getName().equals(PlayerTabOverlayGuiIN.murderAssassinsNick)))
        {
