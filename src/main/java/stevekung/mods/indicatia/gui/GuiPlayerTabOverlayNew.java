...
public class GuiPlayerTabOverlayNew extends GuiPlayerTabOverlay
{
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboard, @Nullable ScoreObjective scoreObjective)
    {
        ...
        for (NetworkPlayerInfo info : list)
        {
            ...
            {
                stringWidth = this.mc.fontRendererObj.getStringWidth(" " + scoreboard.getValueFromObjective(info.getGameProfile().getName(), scoreObjective).getScorePoints());
            ...
            if (k4 < list.size())
            {
                ...
                else
                {
                    for (String hide : HideNameData.getHideNameList())
                    {
                        if (s4.contains(hide))
                        {
                            s4 = s4.replace(hide, EnumChatFormatting.OBFUSCATED + hide + EnumChatFormatting.RESET);
                        }
                    }
                    if (s4.contains("YOU"))
                    {
                        GuiPlayerTabOverlayNew.murderAssassinsNick = EnumChatFormatting.getTextWithoutFormattingCodes(s4).replace("YOU ", "");
                    }
